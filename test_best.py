...
"""
    ...
    overrides = {
        ...
        "optimizer": "AdamW",
        "conf": 0.15,
        # оказалось, что есть люди с 0.27... попробуем еще чуть понизить, чтобы recall повышался, он для нас важнее
        "iou": 0.7,
        # метрики precision, recall, f1-score будут рассчитываться более строго, с порогом 0.7, а не 0.5, как в базовых настройках
        "lr0": 1e-4,  # стартовый learning rate, к нему lr подойдет после warmup
